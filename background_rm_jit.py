...
@jit(nopython=True)
def getvander(x,y,valid,SOPV,vtmp,vtmpmasked,vander,vandermasked,deg,ismask):
    ...
    for yp in range(deg*2+1):
        for xp in range(deg*2+1-yp):
            ...
            if ismask:
                ...
                vtmpmasked=np.multiply(vtmp,valid)
...
def polyfit2dcoeff(im, *,x=None, y=None, deg=2, mask=None, vanderOut=None):
    ...
    if valid is None:
        ismask=False
        valid2=np.empty(im.shape,dtype=np.bool)
    getvander(x,y,valid2,SOPV,vtmp,vtmpmasked,vander,vandermasked,deg,ismask)
